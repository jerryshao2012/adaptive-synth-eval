...
def _write_synth_contract(tmp_path, file_name="contract.json", run_id="run1"):
        ...
        json.dumps(
            {
                ...
                "output": {"base_dir": str(tmp_path / "outputs"), "run_id": run_id},
            }
        )
    )
...
def test_cli_dry_run_end_to_end(tmp_path):
    contract_path = _write_synth_contract(
        tmp_path, file_name="contract.json", run_id="run1"
    )
