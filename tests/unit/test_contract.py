...
def _base_contract(tmp_path, build_synth_contract_payload):
    return build_synth_contract_payload(
        base_dir=tmp_path,
        total_conversations=4,
        turn_min=3,
        turn_max=8,
        num_synthetic_days=7,
        include_legacy_tool_expectations=True,
    )


def test_load_contract_normalizes_defaults_and_warns_for_legacy_tools(
    tmp_path, build_synth_contract_payload
):
    path = tmp_path / "contract.json"
    path.write_text(json.dumps(_base_contract(tmp_path, build_synth_contract_payload)))
...
def test_load_contract_parses_optional_scenario_reference_answer(
    ...
    assert (
        scenario.reference_answer == "Eligibility begins after 90 days of employment."
    )
...
def test_load_contract_rejects_missing_persona_required_field(
    tmp_path, build_synth_contract_payload
):
    payload = _base_contract(tmp_path, build_synth_contract_payload)
...
def test_load_contract_rejects_invalid_turn_range(
    tmp_path, build_synth_contract_payload
):
    payload = _base_contract(tmp_path, build_synth_contract_payload)
...
def test_load_contract_resolves_env_vars_in_endpoint(
    tmp_path, build_synth_contract_payload
):
...
def test_load_contract_resolves_env_vars_without_default(
    tmp_path, build_synth_contract_payload
):
...
def test_load_contract_resolves_env_vars_from_dotenv_file(
    tmp_path, build_synth_contract_payload
):
    payload = _base_contract(tmp_path, build_synth_contract_payload)
    payload["target"] = {
    ...
    dotenv_path.write_text(
        "ASE_TEST_CHATBOT_ENDPOINT=https://dotenv.example.com\n", encoding="utf-8"
    )
...
def test_load_contract_parses_simulator_llm_config(
    tmp_path, build_synth_contract_payload
):
    payload = _base_contract(tmp_path, build_synth_contract_payload)
    payload["llm"] = {
    ...
    assert (
        contract.llm.bedrock_endpoint == "https://bedrock-mantle.us-east-1.api.aws/v1"
    )
    assert contract.llm.ollama_base_url == "http://localhost:11434"


def test_load_contract_resolves_env_vars_in_simulator_llm_config(
    tmp_path, build_synth_contract_payload
):
    payload = _base_contract(tmp_path, build_synth_contract_payload)
    payload["llm"] = {
    ...
    with patch.dict(
        os.environ,
        {"SIM_PROVIDER": "anthropic", "SIM_MODEL": "claude-sonnet-4"},
        clear=False,
    ):
        contract = load_contract(path)
...
def test_load_contract_parses_browser_chatbot_config(
    tmp_path, build_synth_contract_payload
):
    payload = _base_contract(tmp_path, build_synth_contract_payload)
    payload["target"] = {
...
def test_contract_to_dict_serializes_browser_chatbot_config(
    tmp_path, build_synth_contract_payload
):
    payload = _base_contract(tmp_path, build_synth_contract_payload)
    payload["target"] = {
...
def test_load_contract_parses_agentcore_target_config(
    tmp_path, build_synth_contract_payload
):
    payload = _base_contract(tmp_path, build_synth_contract_payload)
    payload["target"] = {
...
def test_contract_to_dict_serializes_agentcore_target_config(
    tmp_path, build_synth_contract_payload
):
    payload = _base_contract(tmp_path, build_synth_contract_payload)
    payload["target"] = {
