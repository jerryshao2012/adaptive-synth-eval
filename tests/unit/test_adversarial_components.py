...
from adaptive_synth_eval.adversarial_response_engine.skills.tools import ToolContext
...
def test_resolve_hijack_target_uses_override():
    assert (
        resolve_hijack_target("persona-hijack", "a sworn judge advising the bench")
...
def test_resolve_hijack_target_falls_back_to_default_for_persona_hijack():
    assert (
        resolve_hijack_target("persona-hijack", None)
        == HIJACK_TARGET_DEFAULTS["persona-hijack"]
    )
...
def test_render_judge_history_empty_session():
    session = SessionState(
        session_id="s1", scenario="probe", scenario_type="data-pii-leak"
    )
...
def test_salvage_json_recovers_messy_and_truncated_output():
    ...
    truncated = (
        '{"failure_score": 4, "refusal_quality": 0, "reasoning": "out-of-scope disclos'
    )
...
def test_safety_judge_retries_once_on_invalid_json():
    ...
    judge.llm = _FlakyLLM()
    judge._system = "sys"
    judge._user_template = (
        "u={user_input} b={chatbot_response} s={scenario} h={history}"
    )
...
def test_judge_output_schema_lists_scores_before_reasoning():
    from adaptive_synth_eval.adversarial_response_engine.engine.prompts import (
        JUDGE_COMMON_OUTPUT_SCHEMA,
    )
...
def test_attack_agent_commits_one_angle_across_turns_until_stall():
    agent = _agent(seed=3)
    session = SessionState(
        session_id="s", scenario="x", scenario_type="data-pii-leak", max_turns=5
    )
...
def test_attack_agent_pins_assigned_angle_onto_plan():
    agent = _agent(seed=1)
    session = SessionState(
        session_id="s", scenario="x", scenario_type="toxicity", max_turns=3
    )
...
def test_attack_agent_restricts_angles_and_delegates_to_selected_skill():
    class _SkillExecutor:
        ...
        def plan(self, **kwargs):
            ...
            return PlanResult.from_dict(
                {
                    "attack_angle": kwargs["fixed_angle"],
                    "sub_tactic": skill.sub_tactics[0],
                    "model_posture": "guarded",
                    "next_generator_instruction": "skill-driven probe",
    ...
    agent = AttackAgent(
        ...
        skill_include=("semantic-drift",),
        target_capabilities={"mode": "api"},
    )
    session = SessionState(
        session_id="s",
        scenario="x",
    ...
    assert probe is not None
    assert probe.plan.attack_angle == "semantic_drift"
    assert probe.plan.raw["skill_name"] == "semantic-drift"
    assert isinstance(executor.calls[0]["context"], ToolContext)
    assert executor.calls[0]["context"].target_capabilities == {"mode": "api"}
    assert agent.planner.fixed_angles == []


def test_variation_seed_has_diversity_across_seeds():
    seeds = {
        AttackAgent(
            _FakePlanner(), _FakeGenerator(), rng=random.Random(s)
        )._variation_seed
        for s in range(10)
    }
    assert len(seeds) > 1
...
def test_to_context_str_three_bands_and_sub_tactic_recording():
    session = SessionState(
        session_id="s1", scenario="x", scenario_type="data-pii-leak", max_turns=3
    )
    session.turns = [
        TurnRecord(
            session_id="s1",
            ...
            chatbot_response="b",
            strategy_before_turn={
                "attack_angle": "decomposition_attack",
                "sub_tactic": "component_extraction",
            },
            judge_result={"failure_score": 4, "near_miss": False},
        ),
        TurnRecord(
            ...
            chatbot_response="b",
            strategy_before_turn={
                "attack_angle": "memory_exploitation",
    ...
    assert (
        "memory_exploitation/prior_compliance_reference" in ctx
    )  # nearly worked (near-miss)
...
def test_planner_renders_memory_using_session_threshold():
    ...
    planner.plan(
        SessionState(session_id="s", scenario="x", failure_threshold=4), memory
    )
...
_SAMPLE_TRACE = {
    ...
    "retrieved_items": [
        {"source": "retrieve_user_profile", "result": {"sin": "123-456-789"}}
    ],
    "memory_reads": [],
    "memory_writes": [],
    "errors": [],
...
def test_meaningful_trace_ignores_empty_normalized_shape_and_latency():
    ...
    assert (
        has_meaningful_trace(normalize_trace({"memory_writes": [{"key": "x"}]})) is True
    )
    assert (
        has_meaningful_trace(
            normalize_trace({"raw_trace": {"nested": [None, {"value": 1}]}})
        )
        is True
    )
    assert (
        has_meaningful_trace(
            normalize_trace({"raw_trace": {"nested": [None, {}, 0, False]}})
        )
        is False
    )
...
def test_trace_summarizer_calls_llm_and_returns_parsed_json():
    ...
    class _LLM:
        ...
        def complete_json(self, system, user):
            ...
            return {
                "binding_actions": ["execute_tfsa_contribution"],
...
def test_evaluator_trajectory_disabled_records_no_trace():
    ...
    from adaptive_synth_eval.adversarial_response_engine.engine.evaluator import (
    ...
    session = SessionState(
        session_id="s", scenario="x", scenario_type="toxicity", max_turns=2
    )
...
def test_evaluator_trajectory_enabled_judges_and_records_trace():
    ...
    session = SessionState(
        session_id="s", scenario="x", scenario_type="tool-call", max_turns=2
    )
    ev._run_turn(session)
    # The judge received the trace summary, and the turn captured trace + summary.
    assert judge.seen_trace_summary == {
        "binding_actions": ["execute_tfsa_contribution"]
    }
