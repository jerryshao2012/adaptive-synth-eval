...
class ProvidedLLM:
    ...
    conversation_factory: Callable[[int], "ProvidedLLM"] | None = None
    prepare_fn: Callable[[], Any] | None = None
    _prepared: bool = field(default=False, init=False, repr=False)

    def for_conversation(self, seed: int) -> "ProvidedLLM":
        """Return isolated scheduling-sensitive state for one conversation."""
        if self.conversation_factory is None:
            return self
        return self.conversation_factory(seed)
...
def build_llm(
    ...
    *,
    ...
    mock_seed: int = 42,
    _with_conversation_factory: bool = True,
) -> ProvidedLLM:
    ...
    if provider == "mock":
        call_fn = (
            make_mock_backend(seed=mock_seed) if need_are else _disabled_are_interface
        )
        synth_chat = (
            _make_mock_synth_chat(mock_seed)
            if need_synth
            else _disabled_synth_interface
        )
        ...
        call_fn = (
            make_claude_backend(
                model=model, api_key=api_key, max_tokens=spec.max_tokens
            )
            if need_are
            else _disabled_are_interface
        )
        synth_chat = (
        ...
        deployment = (
            spec.azure_deployment
            or spec.model
            or os.environ.get("AZURE_OPENAI_DEPLOYMENT", "")
        )
        if not deployment:
            raise ValueError(
                "azure-openai requires azure.deployment or model in contract"
            )
        endpoint = spec.azure_endpoint or os.environ.get("AZURE_OPENAI_ENDPOINT", "")
        api_version = spec.azure_api_version or os.environ.get(
        ...
        region = spec.bedrock_region or os.environ.get(
    ...
    if provider == "mock" and _with_conversation_factory:

        def conversation_factory(seed: int) -> ProvidedLLM:
            return build_llm(
                spec,
                ...
                need_are=need_are,
                need_synth=need_synth,
                mock_seed=seed,
                _with_conversation_factory=False,
            )

    return ProvidedLLM(
        spec=spec,
        call_fn=call_fn,
        synth_chat=synth_chat,
        conversation_factory=conversation_factory,
        prepare_fn=getattr(synth_chat, "prepare", None),
    )
...
def _make_mock_synth_chat(seed: int = 42) -> SynthChatFn:
    ...
    counter = [random.Random(seed).randrange(len(_TEMPLATES))]
...
def _make_bedrock_synth_chat(
    ...
    call.prepare = _client  # type: ignore[attr-defined]
    return call
...
def _make_langchain_synth_chat(
    ...
    *,
    ...
    call.prepare = _build  # type: ignore[attr-defined]
    return call
