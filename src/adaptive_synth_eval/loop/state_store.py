...
def initialize_loop_assets(
    profile: LoopProfile, *, output_dir: Path = Path("outputs")
) -> dict[str, Any]:
    ...
    _write_text_atomic(
        loops_dir / "STATE.md", _render_state_markdown(states, generated_at=timestamp)
    )
    _write_text_atomic(
        loops_dir / "loop-budget.md",
        _render_budget_markdown(states, generated_at=timestamp),
    )
    _append_run_log(
    ...
    *,
    ...
    if state is None:
        raise LoopProfileError(
            f"Loop state not found for profile: {profile.profile_id}"
        )
    ...
    failed_runs = sum(
        1 for item in run_results if str(item.get("status") or "") != "completed"
    )
    ...
    last_cycle = {
        "timestamp": timestamp,
        "ai_reasoning": planner_decision.get("ai_reasoning")
        or "Planner reasoning unavailable.",
        "ai_hypothesis": planner_decision.get("ai_hypothesis"),
        "recommended_action": planner_decision.get("recommended_action")
        or (
            f"Review the latest {total_runs} run summaries and loop run log for follow-up decisions."
        ),
        "checker_decision": checker_decision.get("verdict")
        or "AUTO_APPROVED_REPORT_ONLY",
    ...
    state["human_decision_required"] = bool(
        total_errors
        or failed_runs
        or reflection_decision.get("follow_up_enabled")
        or human_inbox
    )
    state["human_inbox"] = human_inbox[-20:]
    state["updated_at"] = timestamp
    budget = _roll_budget_window(
        state.get("budget") or _initial_budget_state(timestamp), timestamp
    )
    budget["spent_today_runs"] = int(budget.get("spent_today_runs") or 0) + total_runs
    budget["spent_this_week_runs"] = (
        int(budget.get("spent_this_week_runs") or 0) + total_runs
    )
    budget["spent_today_tokens"] = (
        int(budget.get("spent_today_tokens") or 0) + total_tokens
    )
    budget["spent_this_week_tokens"] = (
        int(budget.get("spent_this_week_tokens") or 0) + total_tokens
    )
...
def record_learning_update(
    ...
    *,
    ...
    if error:
        ...
        inbox = list(state.get("human_inbox") or [])
        inbox.append(f"Learning cycle failed: {error}")
        state["human_inbox"] = inbox[-20:]
    ...
    event = (
        f"learning failed error={error}"
        if error
        else f"learning {learning.get('status', 'updated')}"
    )
    _append_run_log(
        loops_dir / "loop-run-log.md",
        profile.profile_id,
...
def _build_initial_state(
    profile: LoopProfile, *, existing: dict[str, Any] | None, timestamp: str
) -> dict[str, Any]:
    if existing is not None:
        ...
        state.setdefault(
            ...
            {
                "status": (
                    "waiting_for_evidence" if profile.learning.enabled else "disabled"
                )
            },
        )
        return state
    ...
    return {
        ...
        else profile.llm_config.__dict__,
...
def _render_state_markdown(states: list[dict[str, Any]], *, generated_at: str) -> str:
    ...
    for state in states:
        ...
        if isinstance(last_cycle, dict):
            lines.append(
                f"- Last cycle timestamp: {last_cycle.get('timestamp', 'unknown')}"
            )
            ...
            if last_cycle.get("recommended_action"):
                lines.append(
                    f"- Recommended action: {last_cycle['recommended_action']}"
                )
...
def _append_run_log(
    path: Path, profile_id: str, event: str, source_path: Path, timestamp: str
) -> None:
